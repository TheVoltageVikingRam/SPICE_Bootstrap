...
\lstdefinestyle{spectrestyle}{
    ...
    commentstyle=\color{steelblue!70}\itshape,
...
\begin{frame}{Part C — $t_{PHL}$ vs $t_{PLH}$ at Symmetric $X = 6.145$ ps}
    \begin{columns}[c]
        \begin{column}{0.48\textwidth}
            \centering
    ...
    \begin{block}{Result}
        ...
        {\footnotesize (Very close to Part A's $k \approx 1.293$ since $X = 6.145$ ps is a short transition time.)}
    \end{block}
\end{frame}

% ── Part C: Rise / Fall Times ───────────────────────────────
\begin{frame}{Part C — $t_{rise}$ vs $t_{fall}$ at Symmetric $X = 6.145$ ps}
    \begin{columns}[c]
        \begin{column}{0.48\textwidth}
            \centering
...
\begin{frame}{Conclusion}
    \begin{block}{Progress}
        Significant progress through Prof.\ Khatri's SPICE exercises. Strong foundation in Spectre control statements, parametric sweeps, and timing analysis established for future PhD research.
    \end{block}

    \vspace{1em}

    \begin{itemize}
        \item Hands-on experience with \texttt{alter}, \texttt{info}, and \texttt{simstat} control statements
        \item Thorough understanding of \texttt{temp} vs \texttt{trise} behaviour
        \item Complete timing characterisation of a 7-stage inverter chain (Parts A--C)
